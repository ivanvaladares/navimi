...
class __Navimi_Core {
    constructor(routes, options, denpendencies) {
        ...
        this._initRoute = async (urlToGo, navParams, force) => {
            ...
            await this._navimiMiddleware.executeMiddlewares(this._abortController, { url, routeItem, params }, (url, params) => {
        ...
        JsClass) => {
            ...
            try {
                ...
                if (this._routesJSsServices[jsUrl] && this._routesJSsServices[jsUrl].length > 0) {
                    while (true) {
                        if (this._routesJSsServices[jsUrl].map((sn) => this._externalJSs[this._options.services[sn]] === undefined).indexOf(true) === -1) {
                            break;
                        }
...
class Navimi {
    constructor(routes, options) {
        ...
        const navimi = new __Navimi_Core(routes, options, {
            navimiFetch,
            navimiJs,
            navimiCss,
            navimiDom,
